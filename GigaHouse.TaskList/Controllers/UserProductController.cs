using AutoMapper;
using GigaHouse.Application.UserProducts.Create;
using GigaHouse.Application.UserProducts.Delete;
using GigaHouse.Application.UserProducts.Get;
using GigaHouse.Application.UserProducts.GetList;
...
namespace GigaHouse.TaskList.Controllers
{
    ...
    public class UserProductController : BaseController
    {
        ...
        public async Task<IActionResult> GetList([FromQuery] int pageNumber = 1, [FromQuery] int pageSize = 10, CancellationToken cancellationToken = default)
        {
            try
            {
                var request = new GetListRequest { PageNumber = pageNumber, PageSize = pageSize, UserId = GetCurrentUserId() };
        ...
        public async Task<IActionResult> Save([FromBody] CreateRequest request, CancellationToken cancellationToken)
        {
            try
            {
                ...
                {
                    Success = true,
                    Message = "UserProduct created successfully",
                    Data = response
                });
            }
            catch (Exception error)
